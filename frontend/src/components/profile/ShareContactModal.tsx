...
const ShareContactModal = ({ open, onOpenChange, sharedContactId }: ShareContactModalProps) => {
  const { conversations, sendDirectMessage, sendGroupMessage } = useChatStore();
  ...
  const handleShare = async () => {
    if (!sharedContactId || selectedConvoIds.length === 0) return;
    
    setIsSending(true);
    try {
      // Create promises for all shares to run in parallel
      const sharePromises = selectedConvoIds.map(convoId => {
      ...
      toast.success("Đã chia sẻ liên hệ");
      onOpenChange(false);
      setSelectedConvoIds([]);
      setSearchQuery("");
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[80vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="px-4 py-3 border-b">
          <DialogTitle>Chia sẻ liên hệ tới</DialogTitle>
        </DialogHeader>
